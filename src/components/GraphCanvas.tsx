...
export const GraphCanvas: React.FC = () => {
  ...
  const [isDirected, setIsDirected] = useState<boolean>(() => getGraphType() === 'directed');
  ...
  const handleGraphTypeChange = useCallback((newIsDirected: boolean) => {
  ...
  const handleVertexClick = useCallback((vertexId: VertexId) => {
    ...
    if (currentMode !== Mode.EDGE) return;
  ...
  }, [currentMode, edgeCreationState, addEdge, removeVertex, edges, isDirected, getEdgeWeight]);
  ...
  const handleWeightClick = useCallback((edgeId: EdgeId, currentWeight: Weight) => {
    while (true) {
      const weightInput = prompt('Enter new edge weight:', currentWeight.toString());
  ...
  const handleMouseDown = useCallback(
    ...
    [positions, currentMode, handleVertexClick]
  );
  ...
  const handleVertexMouseEnter = useCallback((vertexId: VertexId) => {
    if (currentMode === Mode.EDGE && edgeCreationState.sourceVertex && edgeCreationState.sourceVertex !== vertexId) {
      setEdgeCreationState(prev => ({
        ...prev,
  ...
  const renderEdge = (edge: {id: EdgeId, source: VertexId, target: VertexId, weight: Weight}) => {
  ...
  const getInstructionText = () => {
    if (currentMode === Mode.VERTEX) {
      return 'Vertex mode: Click to add • Drag to move vertices';
    }
    ...
    if (edgeCreationState.sourceVertex) {
      ...
      return `Creating edge from ${edgeCreationState.sourceVertex} - click target vertex${weightHint} or ESC to cancel`;
    }
    
    const weightHint = isWeighted ? ' • Weight will be prompted for each edge' : '';
    return `Edge mode: Click first vertex to start edge${weightHint}`;
  };
